...
pll __query_tree(ll node, ll node_start, ll node_end, ll query_start, ll query_end)
{
    ...
    pll left = __query_tree(2 * node, node_start, mid, query_start, query_end);
    pll right = __query_tree(2 * node + 1, mid + 1, node_end, query_start, query_end);
    pll target = {max(left.first, left.second + right.first), left.second + right.second};
    return target;
}
...
void __update_tree(ll node, ll node_start, ll node_end, ll target, ll val)
{
    ...
    {
        seg_tree[node] = {max((ll)0, val), val};
        return;
    }
    ...
    seg_tree[node] = {
        max(
            seg_tree[2 * node].first,
            seg_tree[2 * node].second + seg_tree[2 * node + 1].first),
        seg_tree[2 * node].second + seg_tree[2 * node + 1].second};
}
...
void build_tree(ll &n, vll &arr)
{
    ...
    seg_tree.resize(2 * m);
    forn(i, n)
        seg_tree[m + i] = {max((ll)0, arr[i]), arr[i]};
    rforsn(i, m - 1, 1)
        seg_tree[i] = {
            max(
                seg_tree[2 * i].first,
                seg_tree[2 * i].second + seg_tree[2 * i + 1].first),
            seg_tree[2 * i].second + seg_tree[2 * i + 1].second};
    
    // forn(i, 2 * m)
    // {
    //     cout << i << ": " << seg_tree[i].first << " - " << seg_tree[i].second << endl;
    // }
}

ll query_tree(ll query_start, ll query_end)
{
    return __query_tree(1, 1, seg_tree.size() / 2, query_start, query_end).first;
}
