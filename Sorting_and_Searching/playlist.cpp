...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        int songIndex = lastSongIndex[songId];
        if (songIndex >= sequenceStart)
        {
            longestSequence = max(longestSequence, i - sequenceStart);
            sequenceStart = songIndex + 1;
        }
        lastSongIndex[songId] = i;
    }
    longestSequence = max(longestSequence, n - sequenceStart + 1);
